...
parser.add_argument('--model_name', type=str, default='mlp') # mlp, mlp_small, mlp_large for HAR
...
parser.add_argument('--dataset', type=str, default='har') # har (Human Activity Recognition)
...
def main():
    ...
    # For HAR/MLP models, s2D is None (no depth scaling)
    # For ResNet models, s2D defines the depth structure
    if args.s2D is not None:
        args.num_models = len(args.s2D)  # Should be 2 for ResNet
    else:
        args.num_models = len(args.ps)  # For MLP, use number of width scales
    ...
    for i in range(len(local_models)):
        ...
        # For MLP models, collect BatchNorm1d parameters
        # For ResNet models, collect BatchNorm2d parameters
        for key in w.keys():
            # Collect batch norm parameters (excluding fc.bias)
